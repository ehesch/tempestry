...
fxn_fetch_daily_stations <- function(
  ...
  nearest_weather <- meteo_pull_monitors(monitors = nearby_stations$site$id,
...
fxn_process_daily <- function(.df = x,
  ...
  assign_color_thresholds <- daily_averages %>%
    mutate(temp_category = case_when(tavg <= .temp_extreme_cold ~ "extreme cold",
  ...
  assign_colors <- assign_color_thresholds %>%
    ...
    add_column(color = c(colorRampPalette(RColorBrewer::brewer.pal(9,"Blues")[9])#2 steps darker makes the extremes stand out
...
fxn_plot_daily <- function(.df = x){
  ...
  daily_plot <- .df %>%
    ...
    theme_void() +
    annotate("text", x= (2023 - min_year)/2 + min_year,
