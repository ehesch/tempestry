...
fxn_process_annual <- function(.annual_df,
  ...
  annual_temp_categories <- raw_annual %>%
    mutate(temp_category = case_when(
      temp_anomaly < .extreme_cold_anomaly ~ "extreme cold",
      temp_anomaly <= 0 ~ "cold",
  ...
  annual_colors <- annual_temp_categories %>%
    ...
    mutate(color = c(
      colorRampPalette(RColorBrewer::brewer.pal(9, "Blues")[8]) # while the daily plots use [9] for extreme, the annual plot needs a bit brighter data to stand out against the background
...
fxn_plot_annual <- function(.df = x,
  ...
  annual_plot <- .df %>%
    mutate(
      ...
      nudge = if_else(temp_category == "extreme cold", .cold_label_nudge, .hot_label_nudge)
    ) %>%
    ggplot(aes(
      x = Date, y = temp_anomaly, fill = color,
      label = label
    )) +
